...
## Language Model
To generate song lyrics, I'll develop a statistical language model that utilizes deep learning. This language model will be able to predict the probabiity of the next word in the sequence given an input sequence. To prepare the data, sentences were tokenized using Keras Tokenizer() and padded sequences were created for each sentence. For training, I used an LSTM network or a long short term memory which is a type of RNN with feedback connections. It is capable of learning order dependence in sequence prediction which makes it a good model for text generation because it learns the sentence/line structure and how each word is being arranged to build new sentences. The model currently has an accuracy of 0.495 and loss of 2.052. Below is a graph summarizing the model's training.
